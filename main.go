...
func openPath(l *lua.LState) int {
	mod := l.RegisterModule("path", map[string]lua.LGFunction{
		"join":  pJoin,
		"clean": pClean,
	})
	l.Push(mod)
...
func openFilepath(l *lua.LState) int {
	mod := l.RegisterModule("filepath", map[string]lua.LGFunction{
		"from_slash": fpFromSlash,
		"join":       fpJoin,
		"clean":      fpClean,
		"split":      fpSplit,
...
func openBase64(l *lua.LState) int {
	mod := l.RegisterModule("base64", map[string]lua.LGFunction{
		"encode_file": b64EncodeFile,
		"decode":      b64Decode,
		"encode":      b64Encode,
	})
	l.Push(mod)
